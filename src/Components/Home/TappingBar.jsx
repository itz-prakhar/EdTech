  ...
  return (
    <div className='flex  w-[90%] justify-center gap-3 flex-col items-center mt-24 mb-44'>
      ...
      <p className='text-richblack-300 font-semibold'>Learn to Build Anything You Can Imagine</p>
        <div className='flex flex-wrap justify-center bg-richblack-800 py-2 px-4 mb-20 sm:mb-0 mt-5 sm:gap-5 rounded-full'>
        {
            HomePageExplore.map((data,index)=>(
                <button key={index} className={`text-richblack-300 p-2 sm:px-5 rounded-full hover:text-richblack-25 ${data.tag===selected.tag &&"bg-richblack-900 text-richblack-5"}` } onClick={()=>setSelected(data)} >{data.tag}</button>
            ))
        }
